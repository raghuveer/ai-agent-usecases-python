...
def test_search_text_collects_sources():
    corpus = agent.Corpus()
    obs, sources = corpus.search_text("warranty defects coverage", top_k=2)
...
def test_search_no_match():
    ...
    assert "No matching" in obs and sources == []
...
# --------------------------------------------------------------------------- #
# The StateGraph cycle with a scripted FakeListChatModel
# --------------------------------------------------------------------------- #
def test_graph_runs_search_then_finishes():
...
def test_graph_dedupes_sources():
    fake = FakeListChatModel(
        responses=[
            "Action: search\nAction Input: a",
            "Action: search\nAction Input: b",
