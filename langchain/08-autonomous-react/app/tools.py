...
keyword ``search`` over ``data/facts.md`` — but here they are also exposed as
LangChain ``Tool`` objects so the agent is assembled from LangChain primitives.
...
# --------------------------------------------------------------------------- #
def _calculator_tool_func(expression: str) -> str:
    """Wrap calculator so an UnsafeExpression becomes a tool-visible error."""
    try:
...
def build_tools() -> list[Tool]:
    """Return the agent's tools as LangChain ``Tool`` objects."""
    return [
        Tool(
            name="calculator",
            func=_calculator_tool_func,
            description=(
