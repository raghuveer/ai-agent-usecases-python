...
> **Status — v0.4.0:** all 10 use cases built across all 4 approaches (40 projects + a `_template/` per approach), each with offline, mocked **unit tests** and gated **integration tests**. **512 unit tests green repo-wide, and all 40 projects verified live** against the gateway — then re-verified end to end against the committed lockfiles, so what you install is what was tested. Those live runs caught **ten bugs the mocked tests could not see** — a permission gate that failed *open*; `setting_sources=[]` silently failing to stop the developer's private `~/.claude` memory leaking into agent context; and a gateway that returns `500` on any OpenAI `stop` array over its Anthropic path, which had left both text-ReAct use cases unable to complete a single call. All fixed, each with a regression test. See `TRACKING.md` → Live-run findings. **Security-reviewed** against NIST · OWASP LLM Top 10 · OWASP Web Top 10 (see [`docs/security-review.md`](docs/security-review.md) / [`SECURITY.md`](SECURITY.md)); dependencies patched to current majors, with a CI `pip-audit` gate and Dependabot keeping them current. Examples optimise for clarity of the approach, not production hardening.
